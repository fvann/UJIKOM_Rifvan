                ...
                <tbody>
                    ...
                    <tr class="border-b dark:border-gray-700">
                        ...
                        <td class="px-4 py-3 flex items-center">
                            ...
                            <button id="deleteButton{{ $user->id }}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-red-500 hover:text-red-800 rounded-lg focus:outline-none dark:text-red-400 dark:hover:text-red-100" type="button" onclick="openDeleteModal('{{ $user->id }}')">
    ...
    <div class="modal-content dark:bg-gray-800">
        ...
        <form id="editForm" method="POST" class="px-4 space-y-4">
            ...
            <div>
                ...
                <input type="email" id="edit-email" name="email" autocomplete="email" class="text-gray-700 mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:text-gray-300" value="">
                <!-- Pastikan nama inputnya adalah "email" -->
            </div>
            <div>
                <label for="edit-password" class="block text-sm font-medium text-gray-700">Password</label>
                <input type="password" id="edit-password" name="password" autocomplete="current-password" class="text-gray-700 mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:text-gray-300" value="">
                <!-- Pastikan nama inputnya adalah "password" -->
            </div>
            <div>
    ...
    <div class="modal-content">
        ...
        <form id="addForm" action="{{ route('users.store') }}" method="POST" class="px-4 py-6 space-y-4">
            ...
            <div>
                ...
                <input type="password" id="password" name="password" autocomplete="current-password" class="text-gray-700 mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
            </div>
            <div>
...
<script>
    function openEditModal(id, email, password) {
        var modal = document.getElementById("editModal");
        modal.style.display = "block";
        document.getElementById("edit-email").value = email;
        document.getElementById("edit-password").value = password;
        $('#editForm').attr('action', '/users/' + id); // Set action URL dynamically
    }

    function openDeleteModal(id) {
        var modal = document.getElementById("deleteModal");
        modal.style.display = "block";
        $('#deleteForm').attr('action', '/users/' + id); // Set action URL dynamically
    }
